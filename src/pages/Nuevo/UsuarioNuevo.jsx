...
const TableProductos = ({
  ...
  const addProduct = () => {
    if (data?.PRODUCTO != 0) {
      let existProduct = productsTable?.find(
        (product) => product?.ID_PRODUCTO == data?.PRODUCTO
      );
      if (!existProduct) {
        const productSelected = productosBd?.find(
          (x) => x.ID_PRODUCTO == data?.PRODUCTO
        );
        setProductsTable([
          ...productsTable,
          {
  ...
  const deleteRow = (row) => {
    let totalRow = 0;
    setTotal(0);
    const list = productsTable.filter(
      (product) => product?.ID_PRODUCTO !== row?.ID_PRODUCTO
    );
                    ...
                    {productsTable?.length > 0 ? (
                      productsTable?.map((row, i) => (
                        <TableRow key={row?.ID_PRODUCTO}>
                          ...
                          <TableCell align="center">{`${row?.PRECIO_TOTAL.toFixed(
                            3
                          )}`}</TableCell>
                          <TableCell align="center">
                            <IconButton
